...
(function () {
    ...
    function filterSearch() {
        ...
        for (const rankItem of jquery('.video.matrix')) {
            for (const rankFilter of filters) {
                const filtered = rankFilter.filter(jquery(rankItem));
                if (filtered)
                    break;
            }
        }
        ...
        const mutationCallback = (mutationsList) => {
            for (const mutation of mutationsList) {
                ...
                if (!(mutation.target instanceof Element))
                    continue;
                // 显示搜索结果的Dom元素
                const videoContainerElement = jquery(mutation.target).find('.video-contain.clearfix');
                if (videoContainerElement.length) {
                    for (const item of videoContainerElement) {
                        for (const filter of domChangeFilters) {
                            const filtered = filter.filter(jquery(item));
                            if (filtered)
                                break;
                        }
                    }
    ...
    function filterRank() {
        ...
        for (const rankItem of jquery('.rank-item')) {
